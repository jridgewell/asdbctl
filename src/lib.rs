...
pub fn set_brightness(percent: u8) -> napi::Result<()> {
    set_brightness_internal(percent).map_err(into_js_error)
}

#[napi]
pub fn get_brightness() -> napi::Result<u8> {
    get_brightness_internal().map_err(into_js_error)
}
...
fn get_studio_display() -> Result<DeviceHandle<Context>> {
    rusb::open_device_with_vid_pid(SD_VENDOR_ID, SD_PRODUCT_ID).ok_or(rusb::Error::NoDevice)
}

fn set_brightness_internal(percent: u8) -> Result<()> {
    let mut display = get_studio_display()?;
    let buffer = get_request_data(percent_to_nits(percent));

    display.claim_interface(SD_BRIGHTNESS_INTERFACE)?;
...
fn get_brightness_internal() -> Result<u8> {
    let mut display = get_studio_display()?;
...
#[cfg(test)]
mod tests {
    use crate::{nits_to_percent, percent_to_nits, BRIGHTNESS_MAX, BRIGHTNESS_MIN};
